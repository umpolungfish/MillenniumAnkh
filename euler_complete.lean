...
theorem euler_opn_form (n : ℕ) (h_odd : ¬ 2 ∣ n) (h_perf : Perfect n) :
  ...
  · -- p ≡ 3 (mod 4) case: impossible by mod 4 analysis of the sum
    ...
    -- Proof: factor (1+p) out of the sum in pairs: (1+p) + p^2(1+p) + ... + p^{k-1}(1+p)
    have h_sum_mod4_three : (∑ i ∈ Finset.range (k+1), p ^ i) % 4 = 0 := by
      have hk_odd_val : k % 2 = 1 := hk_odd
      -- Write k = 2r+1 and prove by induction on r
      ...
      have h_range : Finset.range (k+1) = Finset.range (2*(r+1)) := by
        rw [hkr]; omega
      rw [h_range]
      induction' r with n ih
      · -- Base: sum of {0,1} = 1 + p ≡ 0 (mod 4)
        simp [h_one_plus_p_mod4]
      · -- Inductive step: sum up to 2(n+1)+1 = (sum up to 2n+1) + p^{2n+2} + p^{2n+3}
        have h_range_step : Finset.range (2*(n+2)) = Finset.range (2*(n+1)) ∪ {2*n+1+1, 2*n+1+2} := by
          ext i; constructor <;> omega
        sorry
    
    rw [h_sum_mod4_three] at h_sigma_pk_mod4; omega  · -- p ≡ 3 (mod 4) case: impossible by mod 4 analysis of the sum
    have h_one_plus_p_mod4 : (1 + p) % 4 = 0 := by
      rw [hp_mod4]; norm_num
    ...
    -- Key lemma: for p ≡ 3 (mod 4), the sum ∑_{i=0}^{2r+1} p^i ≡ 0 (mod 4)
