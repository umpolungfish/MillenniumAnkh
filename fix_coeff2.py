...
# Replace the broken zify/nlinarith block with a clean "add 332" approach
old = """    have h_coeff : 332 * (2 ^ (e + 1) - 1) > 465 * 2 ^ e := by
...
new = """    have h_coeff : 332 * (2 ^ (e + 1) - 1) > 465 * 2 ^ e := by
      have h2e_ge_2 : 2 ^ e ≥ 2 := by
        calc
          2 ^ e ≥ 2 ^ 1 := Nat.pow_le_pow_right (by norm_num) he_pos
          _ = 2 := by norm_num
      ...
      have h_id : 332 * 2 ^ (e + 1) = 332 * (2 ^ (e + 1) - 1) + 332 := by
        ...
        simp
      rw [h_id] at h_total
      -- h_total : 332*(2^(e+1)-1) + 332 > 465*2^e + 332
      -- Cancel 332 from both sides
      apply Nat.lt_of_add_lt_add_right
      -- Need: (332*(2^(e+1)-1)) + 332 < (465*2^e) + 332  ... wait, h_total is >
...
if old in content:
    ...
    if 'nlinarith' in content:
        print("  nlinarith still in file")
