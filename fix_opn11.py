#!/usr/bin/env python3
"""Move h_sum_even_pairs to a standalone lemma"""
with open("Imscribing/Primitives/OPN_2adic.lean", "r") as f:
    c = f.read()

# Find the h_sum_even_pairs block and replace with a reference to a lemma
...
new = """    have h_sum_even_pairs : ∀ r : ℕ, (∑ i ∈ Finset.range (2*r+2), p ^ i) % 4 = 0 := by
      intro r
      have h_last_two (n : ℕ) : (p ^ (2*n+2) + p ^ (2*n+3)) % 4 = 0 := by
        have h_factor : p ^ (2*n+2) * (1 + p) = p ^ (2*n+2) + p ^ (2*n+3) := by ring
        rw [← h_factor, Nat.mul_mod]
        have h_pow_mod4 : p ^ (2*n+2) % 4 = 1 := by
          rw [show p ^ (2*n+2) = (p ^ 2) ^ (n+1) by ring]
      ...
      | succ n ih =>
        calc
          (∑ i ∈ Finset.range (2*(n+1)+2), p ^ i) % 4
              = (∑ i ∈ Finset.range ((2*n+2)+2), p ^ i) % 4 := by
                omega
