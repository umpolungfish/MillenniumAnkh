...
\textbf{Version}: 0.7 (\pilcrow{}21: Proved $\mathcal{C}_{12}$ instances --- Schwinger/Leray as dimensional gap templates; Goldstone/CMW complementary instances; Witten PE; gap primitive identification machine-checked in PrimitiveBridge.lean \pilcrow{}9)
...
\begin{itemize}
    \item \textbf{\texttt{{[}TOPO{]}}} --- derivable from primitive definitions and composition axioms alone; no physical encoding required; falsified by axiom inconsistency. Worked examples: THREE\_PLANE\_DEMONSTRATION.md \pilcrow{}Yang-Mills TOPO plane.
    ...
    \item \textbf{\texttt{{[}ONTO{]}}} --- ontological or philosophical implication; derivable from TOPO + SCHES results; falsified by showing the structural results do not constrain the admissible ontologies in the claimed way.
\end{itemize}
...
\textbf{Canonical location (SynthOmnicon grammar docs):} TOPO: gap existence theorem $\cdot$ DIAPH: QCD encoding + lattice data $\cdot$ ONTO: confinement as structural necessity
...
\textbf{Canonical location (SynthOmnicon grammar docs):} TOPO: K primitivity theorem $\cdot$ DIAPH: complexity class encodings, K cross-variance data $\cdot$ ONTO: complexity hierarchies as K-primitive hierarchies
...
\textbf{Canonical location (SynthOmnicon grammar docs):} TOPO: H primitive, K-hierarchy temporal theory $\cdot$ DIAPH: cosmological analysis $\cdot$ ONTO: chirality and arrow of time
...
\textbf{Canonical location (SynthOmnicon grammar docs):} TOPO: quantum mechanics as K-tier structure $\cdot$ DIAPH: SM/QG disparity, massless gauge kernel $\cdot$ ONTO: measurement dissolution
...
\begin{itemize}
    ...
    \item \texttt{{[}ONTO{]}} The problem dissolves: ''why does superposition collapse?'' $\to$ ''what determines $R_{\text{dagger}}$ orientation?'' --- a question about constraint context, not physical dynamics; the phenomenological question (what it is \emph{like} to observe) remains on the perpendicular phenomenal axis
\end{itemize}
...
\textbf{Canonical location (SynthOmnicon grammar docs):} TOPO: G-scope tier-crossing cost theorem $\cdot$ DIAPH: numerical calculation, circularity note, generalization table $\cdot$ ONTO: fine-tuning dissolution, physical constants as G-scope readings
...
\textbf{Canonical location (SynthOmnicon grammar docs):} TOPO: G-scope tier-crossing cost theorem (same as \pilcrow{}7) $\cdot$ DIAPH: Higgs calculation, 0.6\% agreement $\cdot$ ONTO: naturalness reframed, no SUSY/extra-dim solution needed
...
BSD_conjecture = \langleD_holo;    T_bowtie; R_lr;    P_pm_sym; F_eth;  K_mod;  G_aleph; \Gamma_and; Phi_c;   H1; 1:1; \Omega_Z2\rangle
...
The new theorem \texttt{rh\_leyang\_structural\_correspondence} (proved by \texttt{rfl}) confirms that $\zeta$ zeros and Lee-Yang partition-function zeros share \texttt{Phi\_c\_complex}. The structural distance of 5 between the two encodings identifies the polarity mismatch ($P_\text{neutral}$ vs $P_{\psi}$) as the structural explanation for why the Lee-Yang theorem is proved but RH remains open.
...
\textbf{Observation}: O$_{\infty}$ requires $H_{\infty}$ --- infinite recursion depth. It is structurally unreachable for any finite system. All finite physical systems are bounded above by $\mathcal{O} = 4$.
...
\midrule
\rowcolors{1}{white}{gray!10}
$(\Phi_c^{\mathbb{C}}, P_{\psi})$ & Zeros of partition function lie on symmetry axis of $P_{\psi}$ &  \textbf{Proved} (Lee-Yang 1952) \\
...
The Lee-Yang proof succeeds in step 4 because $P_{\psi}$ encodes an \emph{explicit} symmetry that directly reflects across the zero locus. The proof is essentially: the symmetry $P_{\psi}$ forces $\mathcal{C}_{13}$ to be a single line, and the zeros must lie on it.

The RH constraint map $\mathcal{C}_{13}(\Phi_c^{\mathbb{C}}, P_\text{neutral})$ fails (so far) at step 4 because $P_\text{neutral}$ encodes an \emph{implicit} symmetry --- the functional equation $s \mapsto 1-s$ exists and is proved ($P$ is present) but does not manifest directly as a forcing mechanism on the zero locus ($P$ is not $P_{\psi}$). The structural gap between $P_\text{neutral}$ and $P_{\psi}$ is precisely the gap between open RH and proved Lee-Yang --- not a gap in analytic technique, but a gap in the \emph{manifestation level} of the symmetry.
...
The $\mathcal{C}_{12}$ gaps are \textbf{minimal} (distance 1); the $\mathcal{C}_{13}$ gap is broader (distance 5 total, 1 essential). Yet both $\mathcal{C}_{12}$ conjectures remain open. This illustrates that \textbf{structural distance to the template does not measure proof difficulty} --- a single primitive gap ($D_\text{wedge} \to D_\text{cube}$) can be harder to close than a five-primitive gap when the blocking field is Dimensionality.
...
\midrule
\rowcolors{1}{white}{gray!10}
$\mathcal{C}_{13}$ & Lee-Yang $(P_{\psi})$ & \textbf{Polarity}: $P_{\psi} \to P_\text{neutral}$ & RH \\
